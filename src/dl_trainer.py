...
def train_model(model, train_loader, val_loader, config, seed):
    ...
    Sınıf dengesizliğini pos_weight ile dinamik olarak dengeler.
    """
    set_seed(seed)
    ...
    # Sınıf dengesizliğini çözmek için pos_weight dinamik hesaplanır.
    # Örn. BATADAL'da ~%90 Normal, %10 Anomali → pos_weight ≈ 9.0
    # Aşırı dengesizlikte patlamayı önlemek için 10.0 ile sınırlandırılır.
    all_y = []
    ...
    for epoch in range(epochs):
        ...
        for X_batch, y_batch in train_loader:
            ...
            # Pozitif sınıfa (Anomali) pos_weight ağırlığı ver
